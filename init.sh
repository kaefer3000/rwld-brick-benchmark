...
for ((cnt=0;cnt<$BUILDINGCOUNT;cnt++)); do
  ...
  awk -f "scripts/sosa-actsOnProperty-for-things.awk" -v base="http://$HOSTNAME:$( expr 40300 + $cnt )/" "$TMPDIR"/IBM_B3-luminance-commands.tsv \
  ...
  awk -f "scripts/sosa-observes-for-things.awk" -v base="http://$HOSTNAME:$( expr 40300 + $cnt )/" "$TMPDIR"/IBM_B3-luminance-alarms.tsv \
  ...
  awk -f "scripts/sosa-observes-for-things.awk" -v base="http://$HOSTNAME:$( expr 40300 + $cnt )/" "$TMPDIR"/IBM_B3-occupancy-sensors.tsv \
  ...
  awk -f "scripts/sosa-observes-for-things.awk" -v base="http://$HOSTNAME:$( expr 40300 + $cnt )/" "$TMPDIR"/IBM_B3-luminance-sensors.tsv \
